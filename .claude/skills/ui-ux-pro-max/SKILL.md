...
> **Local install note (not upstream):** this skill is installed as a *project*
> skill (`.claude/skills/`), not as a plugin, so `CLAUDE_PLUGIN_ROOT` is unset and
> the upstream `${CLAUDE_PLUGIN_ROOT}/...` paths resolve to `/` and fail. The
> commands below use the repo-relative path instead — run them from the project
> root. Everything else is upstream v2.11.0, unmodified.
